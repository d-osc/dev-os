...
```
+------------+------+------------------+--------+------------------+----------------+
| >_ DEVOS   |      |  [D]   [S]      | ))) (S | 21:40:12        |  <- ปุ่ม start  pill ขอบเขียว
+------------+------+------------------+---+----+-----+----------+----------------+
   ^ pill โค้งขอบเขียว     ^ ไอคอนแอปตรงกลาง     ^ tray      ^ เส้นคั่น + นาฬิกา 2 บรรทัด
     ตัวอักษร mono หา        (จุด=กำลังรัน,           (wifi เขียว,   (เวลา 13px ขาว / วันที่
     เขียว คลิกเปิดเมนู         เส้นใต้น้ำเงิน=active)    ลำโพง, กระดิ่ง+  8.5px เทา ตัวพิมพ์ใหญ่)
                                                     จุดแดง)
```
...
- กลางจอเป็นกลุ่มไอคอนแอปที่ติดตั้ง (สี่เหลี่ยมมุมโค้ง 28px พร้อม monogram ตัวแรกของชื่อ)
  จัดกึ่งกลาง สูงสุด 9 ตัว: แอปที่มีหน้าต่างเปิดอยู่มีจุดสีขาวใต้ไอคอน ตัวแรกที่เปิดได้
  เส้นใต้สีน้ำเงิน (`#00aaff`) และ monogram สีขาวสว่าง; คลิกไอคอน = ยกหน้าต่างนั้นขึ้น
  หรือถ้ายังไม่เปิด จะเด้งไปแถวยินยอมของแอปนั้นในเมนูทันที (การจับคู่หน้าต่างใช้ชื่อ
  title ผ่าน `_NET_CLIENT_LIST` แบบ best-effort)
