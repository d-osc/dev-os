...
Pure X11 through ctypes/libX11, the same stack as the DPK window runtime; no
toolkit dependency. The bar reserves the bottom 24 pixels of the screen with
EWMH struts, shows an application menu at the left, running-window buttons
when the window manager publishes _NET_CLIENT_LIST, and the local date/time
at the right. Launching an app needs an explicit per-launch consent in the
menu and then runs `dev launch <name> --allow <declared permissions>`: the
...
MENU_WIDTH = 320
MENU_ITEM_HEIGHT = 34
MENU_HEADER_HEIGHT = 22
CONSENT_HEIGHT = 44
ALLOW_X, CANCEL_X = 214, 288
COLORS = {'bar': 0x142034, 'text': 0xe5edf8, 'accent': 0x65e0bf,
          'dim': 0x8fa3bf, 'hover': 0x1e304d, 'panel': 0x101b2c}
...
def truncate(text, measure, max_px):
    """Shorten text to max_px using measure(text)->pixels, ASCII only."""
...
class XCreate(c.Structure):
    _fields_ = [('type', c.c_int), ('serial', c.c_ulong), ('send_event', c.c_int),
                ('display', c.c_void_p), ('parent', c.c_ulong), ('window', c.c_ulong),
                ('x', c.c_int), ('y', c.c_int), ('width', c.c_int), ('height', c.c_int),
                ('border_width', c.c_int), ('override_redirect', c.c_int)]


class XEvent(c.Union):
    _fields_ = [('any', XAny), ('button', XButton), ('client', XClient),
                ('create', XCreate), ('pad', c.c_long * 24)]
...
def connect():
    ...
    api = {
        ...
        'set_font': bind('XSetFont', c.c_int, c.c_void_p, c.c_void_p, c.c_void_p),
        'set_foreground': bind('XSetForeground', c.c_int, c.c_void_p, c.c_void_p, c.c_ulong),
        'draw_string': bind('XDrawString', c.c_int, c.c_void_p, c.c_ulong, c.c_void_p,
                            c.c_int, c.c_int, c.c_char_p, c.c_int),
        'fill_rectangle': bind('XFillRectangle', c.c_int, c.c_void_p, c.c_ulong, c.c_void_p,
                               c.c_int, c.c_int, c.c_uint, c.c_uint),
        'draw_line': bind('XDrawLine', c.c_int, c.c_void_p, c.c_ulong, c.c_void_p,
        ...
        'load_font': bind('XLoadQueryFont', c.c_void_p, c.c_void_p, c.c_char_p),
        'font_width': bind('XTextWidth', c.c_int, c.c_void_p, c.c_char_p, c.c_int),
        ...
        'set_override_redirect': bind('XChangeWindowAttributes', c.c_int, c.c_void_p, c.c_ulong,
...
def run(root, *, dev='dev', shots=None):
    ...
    bar = api['create'](display, root_window, 0, height - BAR_HEIGHT, width, BAR_HEIGHT,
                        1, COLORS['bar'], COLORS['bar'])
    api['store_name'](display, bar, b'Dev OS Shell')
    menu = api['create'](display, root_window, 0, 0, 100, 100, 1, COLORS['panel'], COLORS['panel'])
    api['store_name'](display, menu, b'Dev OS Menu')
    if shots is None:
        # Override-redirect keeps the interactive popup undecorated; XWayland
        # cannot XGetImage such windows, so capture mode uses a normal window.
        attributes = XSetWindowAttributes(override_redirect=1)
        api['set_override_redirect'](display, menu, 1 << 9, c.byref(attributes))
    ...
    font = api['load_font'](display, b'fixed')
    if not font:
        raise SystemExit('The X server has no "fixed" font')
    gc_bar = api['create_gc'](display, bar, 0, None)
    gc_menu = api['create_gc'](display, menu, 0, None)
    api['set_font'](display, gc_bar, font)
    api['set_font'](display, gc_menu, font)

    def measure(text):
        return api['font_width'](font, text.encode('ascii', 'replace'), len(text))

    def write(target, gc, text, px, py, color):
        api['set_foreground'](display, gc, color)
        encoded = text.encode('ascii', 'replace')
        api['draw_string'](display, target, gc, px, py, encoded, len(encoded))
    ...
    def draw_bar():
        nonlocal clock
        api['set_foreground'](display, gc_bar, COLORS['bar'])
        api['fill_rectangle'](display, bar, gc_bar, 0, 0, width, BAR_HEIGHT)
        api['set_foreground'](display, gc_bar, COLORS['accent'])
        api['fill_rectangle'](display, bar, gc_bar, 0, BAR_HEIGHT - 2, MENU_BUTTON_WIDTH, 2)
        write(bar, gc_bar, 'MENU', 10, 16, COLORS['accent'])
        left = MENU_BUTTON_WIDTH + 1
        tasks = clients()
        room = max(0, (width - MENU_BUTTON_WIDTH - measure(clock_text()) - 40) // TASK_BUTTON_WIDTH)
        for title, window in tasks[:room]:
            api['set_foreground'](display, gc_bar, COLORS['hover'])
            api['fill_rectangle'](display, bar, gc_bar, left, 4, TASK_BUTTON_WIDTH - 8, BAR_HEIGHT - 8)
            write(bar, gc_bar, truncate(' '.join(title.split()), measure, TASK_BUTTON_WIDTH - 24)
                  or 'window', left + 6, 16, COLORS['text'])
            left += TASK_BUTTON_WIDTH
        write(bar, gc_bar, clock := clock_text(), width - measure(clock) - CLOCK_PAD, 16,
              COLORS['text'])
        api['flush'](display)
        return tasks

    def draw_menu():
        api['set_foreground'](display, gc_menu, COLORS['panel'])
        api['fill_rectangle'](display, menu, gc_menu, 0, 0, MENU_WIDTH, height)
        write(menu, gc_menu, 'DEV OS', 10, 15, COLORS['accent'])
        api['set_foreground'](display, gc_menu, COLORS['dim'])
        api['draw_line'](display, menu, gc_menu, 0, MENU_HEADER_HEIGHT - 2, MENU_WIDTH,
                         MENU_HEADER_HEIGHT - 2)
        top = MENU_HEADER_HEIGHT
        for item in items:
            write(menu, gc_menu, item['label'], 12, top + 15, COLORS['text'])
            detail = ' · '.join(item['categories'][:2] +
                                [','.join(item['permissions']) or 'no permissions'])
            write(menu, gc_menu, truncate(detail, measure, MENU_WIDTH - 28), 24, top + 29,
                  COLORS['dim'])
            top += MENU_ITEM_HEIGHT
        if consent:
            api['set_foreground'](display, gc_menu, COLORS['hover'])
            api['fill_rectangle'](display, menu, gc_menu, 0, top, MENU_WIDTH, CONSENT_HEIGHT)
            write(menu, gc_menu, 'Open ' + truncate(consent['label'], measure, 180) + '?',
                  12, top + 16, COLORS['text'])
            write(menu, gc_menu, truncate(grant_for(consent), measure, 300) or 'no permissions',
                  12, top + 32, COLORS['dim'])
            write(menu, gc_menu, '[ ALLOW ]', ALLOW_X, top + 24, COLORS['accent'])
            write(menu, gc_menu, '[ cancel ]', CANCEL_X, top + 24, COLORS['dim'])
        api['flush'](display)
    ...
    while running:
        ...
        while api['pending'](display):
            ...
            if (kind == 33 and event.client.message_type == protocols
                ...
                kind_hit, index = bar_hit(width, clock, event.button.x, len(tasks))
                if kind_hit == 'menu':
                    open_menu(not menu_open)
                elif kind_hit == 'task' and index < len(tasks):
        ...
        if not captured and time.monotonic() - started >= 1.0:
            ...
            draw_menu()
            api['sync'](display, 0)
            shot_height = min(menu_geometry(width, height, len(items), consent is not None)['height'],
                              MENU_HEADER_HEIGHT + max(1, len(items)) * MENU_ITEM_HEIGHT +
                              (CONSENT_HEIGHT if consent else 0))
            screenshot(x, api, display, menu, shots[1],
                       menu_geometry(width, height, 0)['width'], shot_height)
            running = False
        time.sleep(0.2)
