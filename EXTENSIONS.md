...
dev ext dedup                         # ตรวจและปิดสำเนาซ้ำของ extension เดียวกัน
```

**การตรวจงานซ้ำซ้อนอัตโนมัติ**: ถ้า extension id เดียวกันถูกเปิดอยู่สองสำเนา
(เช่น ติดตั้ง battery ซ้ำกับของที่มากับ image) นั่นคือคำสั่ง/ไอคอน/widget ซ้ำสองเท่า
— และถ้าเป็น extension ภาษา JS คือ Node process สองตัว — shell จะตรวจตอนโหลด
และ **disable สำเนารุ่นเก่ากว่าถาวร** (เทียบ MAJOR.MINOR.PATCH; เวอร์ชันเท่ากัน
ให้สำเนาของผู้ใช้ชนะ ตามธรรมเนียม settings) พร้อมรายงานผ่าน stderr ของ shell
และรันเองได้ด้วย `dev ext dedup` — สำเนาที่อยู่ใน directory ระบบและเขียน marker
ไม่ได้จะถูกข้ามเฉพาะ session นั้น (ต้อง sudo ถ้าจะปิดถาวร)
